...
export async function handleLogin(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (errorMsgDiv) {
                errorMsgDiv.textContent = result.message || "Неверное имя пользователя или пароль";
        ...
        if (errorMsgDiv) {
            errorMsgDiv.textContent = "Не удалось связаться с сервером";
            errorMsgDiv.style.display = 'block';
        } else {
...
export async function handleSignup(event) {
    ...
    if (username.length < 3 || password.length < 4) {
        if (errorMsgDiv) {
            errorMsgDiv.textContent = "Никнейм от 3 симв., пароль от 4 симв.!";
            errorMsgDiv.style.display = 'block';
        } else {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (errorMsgDiv) {
                errorMsgDiv.textContent = result.message || "Ошибка регистрации";
                errorMsgDiv.style.display = 'block';
            } else {
